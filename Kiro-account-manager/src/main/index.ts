...
import { generateColabProxy, type GenerateColabProxyConfig } from './registration/colab-proxy'
import {
  createTray,
  destroyTray,
  updateTrayMenu,
...
// 获取网络代理 agent。K-Proxy 开启时优先通过 K-Proxy MITM；否则使用用户/系统代理。
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        const SOCIAL_PROFILE_ARN =
          'arn:aws:codewhisperer:us-east-1:699475941385:profile/EHGA3GRVQMUK'
        const BUILDER_ID_PROFILE_ARN =
          'arn:aws:codewhisperer:us-east-1:638616132270:profile/AAAACCCCXXXX'
        const resolvedProfileArn =
          profileArn ||
          (authMethod === 'social' || provider === 'Google' || provider === 'Github'
            ? SOCIAL_PROFILE_ARN
