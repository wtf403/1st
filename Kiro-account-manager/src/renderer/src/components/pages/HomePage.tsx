...
export function HomePage() {
  ...
  useEffect(() => {
    let cancelled = false
    const loadProxyStats = async (): Promise<void> => {
      try {
        const result = (await window.api.proxyGetStatus()) as ProxyStatusResult
        if (!cancelled) {
          setProxyRunning(result.running)
          setProxyStats(result.stats || null)
        }
      } catch {
        if (!cancelled) {
    ...
    loadProxyStats()
    const timer = window.setInterval(loadProxyStats, 5000)
  ...
  const proxyCost = proxyStats?.totalCost || 0
  const proxyTotalTokens = (proxyStats?.inputTokens || 0) + (proxyStats?.outputTokens || 0)
        ...
        <div className="relative flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-primary">
              {isEn ? 'Welcome to Kiro Account Manager' : '欢迎使用 Kiro 账户管理器'}
            </h1>
              ...
              <div className="p-3 bg-muted rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {isEn ? 'Estimated Cost' : '预估费用'}
                  </span>
                </div>
                <p className="text-xl font-bold text-green-600">{formatCurrency(proxyCost)}</p>
              </div>
              <div className="p-3 bg-muted rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  <Activity className="h-4 w-4 text-indigo-500" />
                  <span className="text-xs text-muted-foreground">
                    {isEn ? 'Total Tokens' : '总 Tokens'}
                  </span>
                </div>
                <p className="text-xl font-bold">{proxyTotalTokens.toLocaleString()}</p>
              </div>
              <div className="p-3 bg-muted rounded-lg">
                <div className="flex items-center gap-2 mb-1">
                  <TrendingUp className="h-4 w-4 text-blue-500" />
                  <span className="text-xs text-muted-foreground">
                    {isEn ? 'Input / Output' : '输入 / 输出'}
                  </span>
                </div>
                <p className="text-sm font-bold">
                  <span className="text-blue-500">
                    {(proxyStats?.inputTokens || 0).toLocaleString()}
                  </span>
                  <span className="text-muted-foreground mx-1">/</span>
                  <span className="text-purple-500">
                    {(proxyStats?.outputTokens || 0).toLocaleString()}
                  </span>
                </p>
              </div>
              <div className="p-3 bg-muted rounded-lg">
                ...
                <p className="text-sm font-bold">
                  <span className="text-green-500">{proxyStats?.successRequests || 0}</span>
                  <span className="text-muted-foreground mx-1">/</span>
                  <span className="text-red-500">{proxyStats?.failedRequests || 0}</span>
                </p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground mt-3">
              {isEn
                ? 'Cost is calculated from input/output token counts using the configured model price table.'
                : '费用按模型价格表和输入/输出 Token 数计算。'}
            </p>
          </CardContent>
        </Card>
      )}
          ...
          <CardContent>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-muted-foreground">
                ...
                <span>
                  {usageStats.totalUsed.toLocaleString()} / {usageStats.totalLimit.toLocaleString()}
                </span>
              </div>
            ...
            <div className="pt-3 border-t space-y-2">
              ...
              <div className="space-y-1.5 text-xs">
                <div className="flex items-start gap-2">
                  ...
                  <span className="font-mono text-[10px] break-all select-all">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {isEn
                    ? 'Modify device ID, auto-switch, account binding'
